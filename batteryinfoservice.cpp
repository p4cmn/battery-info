...
void BatteryInfoService::fetchBatteryInfo() {
    ...
    if (GetSystemPowerStatus(&systemPowerStatus)) {
        updateBatteryChargeLevel(systemPowerStatus);
        updatePowerMode(systemPowerStatus);
        updateRemainingBatteryLifetime(systemPowerStatus);
        updatePowerSupplyType(systemPowerStatus);
        handleChargingStatus(systemPowerStatus);
        updateDischargeTime();
    }
...
void BatteryInfoService::updateBatteryChargeLevel(const SYSTEM_POWER_STATUS &systemPowerStatus) {
    emit batteryChargeLevelChanged(systemPowerStatus.BatteryLifePercent);
}

void BatteryInfoService::updatePowerMode(const SYSTEM_POWER_STATUS &systemPowerStatus) {
    PowerMode mode = convertToPowerMode(systemPowerStatus.Reserved1);
    emit powerModeChanged(mode);
}

void BatteryInfoService::updateRemainingBatteryLifetime(const SYSTEM_POWER_STATUS &systemPowerStatus) {
...
void BatteryInfoService::updatePowerSupplyType(const SYSTEM_POWER_STATUS &systemPowerStatus) {
    PowerSupplyType type = convertToPowerSupplyType(systemPowerStatus.ACLineStatus);
    emit powerSupplyTypeChanged(type);
}

void BatteryInfoService::handleChargingStatus(const SYSTEM_POWER_STATUS &systemPowerStatus) {
...
void BatteryInfoService::updateDischargeTime() {
...
void BatteryInfoService::fetchBatteryDetails(HANDLE hBattery, BATTERY_QUERY_INFORMATION &bqi) {
    ...
    if (DeviceIoControl(hBattery, IOCTL_BATTERY_QUERY_INFORMATION, &bqi, sizeof(bqi),
        ...
        emit batteryMaxCapacityChanged(static_cast<double>(bi.DesignedCapacity));
        emit batteryCurrentCapacityChanged(static_cast<double>(bi.FullChargedCapacity));
    }
}
