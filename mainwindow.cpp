...
MainWindow::MainWindow(BatteryInfoModel* sourceModel, QWidget *parent)
    : QMainWindow(parent),
    ui(new Ui::MainWindow),
    model(sourceModel),
    systemControlService(new SystemControlService(this)),
    systemControlController(new SystemControlController(systemControlService, this)),
    trayIcon(nullptr),
    trayMenu(nullptr) {

    ui->setupUi(this);
    connect(sourceModel, &BatteryInfoModel::dataUpdated, this, &MainWindow::updateView);
    updateView();
...
void MainWindow::updateView() {
    int chargeLevel = model->getChargeLevel();
    updateChargeLevelProgressBar(chargeLevel);
    updatePowerMode(model->getPowerMode());
    updateRemainingBatteryLifetime(model->getRemainingBatteryLifetime());
    updatePowerSupplyType(model->getPowerSupplyType());
    updateDischargeTime(model->getDischargeTime());

    updateBatteryType(model->getBatteryType());
    updateMaxCapacity(model->getMaxCapacity());
    updateCurrentCapacity(model->getCurrentCapacity());
    updateVoltage(model->getVoltage());
    updateBatteryHealth(model->getBatteryHealth());
}
...
void MainWindow::handleSleepButtonClicked() {
    systemControlController->onSleepButtonClicked();
}

void MainWindow::handleHibernateButtonClicked() {
    systemControlController->onHibernateButtonClicked();
}
